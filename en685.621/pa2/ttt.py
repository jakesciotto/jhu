...
# make_move
#
# Sets the board state for 2 if the AI
# makes the move, and 1 if the player 
# makes the move
########################################
def make_move(boardState, move, ai_move):
    if ai_move:
        boardState[move] = 2
    else:
        boardState[move] = 1
...
# undo_move
#
# Un-does the perfomed move
########################################
def undo_move(boardState, move):
    boardState[move] = 0

########################################
# get_moves
#
# Enumerates all moves possible from the
# board state
########################################
def get_moves(boardState):
    return [i for i, e in enumerate(boardState) if e == 0]

########################################
# minimax_ab
#
# Performs minimax with alpha beta
# pruning
########################################
def minimax_ab(maximizing, ai_move, boardState, alpha, beta):
    state = checkWin2(boardState)
    ...
    if state is None:
        ...
        for move in get_moves(boardState):
            make_move(boardState, move, ai_move)
            score, _ = minimax_ab(not maximizing, not ai_move, boardState, alpha, beta)
            undo_move(boardState, move)
...
# minimax
#
# Performs regular minimax algorithm
########################################
def minimax(maximizing, ai_move, boardState):
    ...
    if state is None:
        ...
        for move in get_moves(boardState):
            make_move(boardState, move, ai_move)
            score, _ = minimax(not maximizing, not ai_move, boardState)
...
# decisionMaker
#
# Performs minimax based on true/false
# value of minimax_ variable
########################################
def decisionMaker(boardState, minimax_, depth):

    ai_move = True

    if minimax_:
        score, move = minimax(True, ai_move, boardState)
    else:
        alpha = -math.inf
        beta = math.inf
        score, move = minimax_ab(True, ai_move, boardState, alpha, beta)
...
def decisionMaker(boardState, depth, player):
    ...
    # iterate through possible moves and states
    for i in range(9):
        if boardState[i] == 0:

            # perform minimax
            best = minimax(boardState, depth, player)
         
            # return best move if it is the best score
            if best > score:
                score = best
                best_move = i
                return best_move
...
# Performs regular minimax algorithm
########################################
def minimax(place, depth, turn):

    # tie
    if checkWin2(place) == 0:
        return 0
    # computer win
    if checkWin2(place) == 2: 
        return -1 * (place.count(0) + 1)
    # player win
    ...
    if turn == 0:
        ...
        best = -math.inf
        for i in range(9):
            # if move available, set 2 and recurse
            if place[i] == 0:
                place[i] = 2
                scores.append(minimax(place, depth + 1, 1))
                # undo move
                place[i] = 0
        return min(scores)
    ...
    if turn == 1:
        ...
        best = math.inf
        for i in range(9):
            # if move available, set 1 and recurse
            if place[i] == 0:
                place[i] = 1
                scores.append(minimax(place, depth + 1, 0))
                # undo move
                place[i] = 0 
        return max(scores)
